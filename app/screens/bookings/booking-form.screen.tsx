...
export default function BookingFormScreen({ route }: any) {
  ...
  useEffect(() => {
    if (route?.params?.pickupLocation) formState.setPickupLocation(route.params.pickupLocation)
    if (route?.params?.dropoffLocation) formState.setDropoffLocation(route.params.dropoffLocation)
  }, [route?.params])
      ...
      if (user?.id) {
        try {
          ...
          if (userResult.data) {
            ...
            formState.setName(userData.fullname || userData.username || user.name || "")
  ...
  const handleNextStep = async () => {
    if (currentStep === 1) {
      ...
      if (validation.validateStep4(formState.agreeMarketing, formState.agreeTerms)) {
        handleCreate()
      }
    }
  }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!formState.pickupLocation.trim() || !formState.dropoffLocation.trim()) {
      ...
      // Prepare booking data with explicit type checking
      const bookingData = {
        ...
        bookingFee: 15, // Minimum booking fee required by backend
      ...
      // If first attempt fails with 500 error, try without bookingFee
      ...
      if (res.data) {
        ...
        if (typeof bookingResponse === 'string') {
          ...
          if (bookingResponse.booking && bookingResponse.booking.id) createdBookingId = bookingResponse.booking.id
          if (!createdBookingId) createdBookingId = bookingResponse.bookingId || bookingResponse.id || null
          if (!payosUrl) payosUrl = bookingResponse.paymentUrl || bookingResponse.checkoutUrl || null
        }
  ...
  const getCarImageSource = () => {
    if (car.imageUrls && car.imageUrls.length > 0) return { uri: car.imageUrls[0] }
    if (car.images && car.images.length > 0) return { uri: car.images[0] }
    if (car.image) {
      if (car.image.startsWith('http://') || car.image.startsWith('https://')) return { uri: car.image }
      ...
      <View style={{ flex: 1, backgroundColor: colors.background }}>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 24 }} keyboardShouldPersistTaps="handled">
          ...
          <View style={{ paddingHorizontal: 16, marginBottom: 16 }}>
            <Text style={{ fontSize: 12, color: colors.placeholder }}>{t("step")} {currentStep} {t("of")} 4</Text>
          </View>
          ...
          <View style={{ flexDirection: "row", gap: 12, paddingHorizontal: 16, paddingTop: 24 }}>
            {currentStep > 1 && (
              <Pressable
                onPress={() => setCurrentStep(currentStep - 1)}
                style={{ flex: 1, borderWidth: 1, borderColor: colors.morentBlue, paddingVertical: 12, borderRadius: 8, alignItems: "center" }}
              >
                <Text style={{ color: colors.morentBlue, fontWeight: "600" }}>{t("back")}</Text>
              </Pressable>
            )}
            <Pressable
              onPress={handleNextStep}
              disabled={loading}
              style={{ flex: 1, backgroundColor: colors.morentBlue, paddingVertical: 12, borderRadius: 8, alignItems: "center" }}
            >
              {loading ? (
                <ActivityIndicator color={colors.white} />
              ) : (
                <Text style={{ color: colors.white, fontWeight: "600" }}>{currentStep === 4 ? t("rentalNow") : t("next")}</Text>
              )}
            </Pressable>
          </View>
        </ScrollView>
