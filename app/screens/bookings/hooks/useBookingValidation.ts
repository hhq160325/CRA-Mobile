...
export function useBookingValidation() {
    const validateDateTime = (date: string, time: string, fieldName: string): Date | null => {
        ...
        // Ensure time has leading zeros for proper parsing
        const [hours, minutes] = time.split(':')
        const paddedTime = `${hours.padStart(2, '0')}:${minutes.padStart(2, '0')}`

        // Create datetime string in ISO format for consistent parsing
    ...
    const validateStep1 = (name: string, address: string, phone: string, city: string): boolean => {
        if (!name.trim()) {
    ...
    const validateStep4 = (agreeMarketing: boolean, agreeTerms: boolean): boolean => {
        if (!agreeMarketing) {
