...
export const useProfileActions = (
    ...
    const uploadDriverLicense = async (uri: string) => {
        ...
        try {
            ...
            const { data: licenseData, error: licenseError } = await userService.getDriverLicense(userId, userEmail);
            ...
            if (data && data.imageAvatar) {
                ...
                try {
                    ...
                    if (userStr) {
                        ...
                        // Update both avatar and imageAvatar fields
                        currentUser.avatar = data.imageAvatar;
                        currentUser.imageAvatar = data.imageAvatar;
                        localStorage.setItem("user", JSON.stringify(currentUser));
                        console.log("Updated user in localStorage with new avatar:", data.imageAvatar);
                    }
                } catch (err) {
                    console.error("Failed to update localStorage:", err);
                }

                // Force refresh the user context to trigger re-render
                setTimeout(() => {
                    refreshUser();
                }, 100);
            }
