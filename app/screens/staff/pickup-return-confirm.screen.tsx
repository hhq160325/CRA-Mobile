...
export default function PickupReturnConfirmScreen() {
    ...
    const { booking, loading, error, isAlreadyCheckedIn, existingCheckInData, initialDescription } = usePickupConfirm(bookingId)
    const { selectedImages, showImagePickerOptions, removeImage } = useImagePicker(5)
    const [description, setDescription] = useState(initialDescription)
    const [submitting, setSubmitting] = useState(false)

    // Update description when initial data loads
    useEffect(() => {
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                <ImageGallerySection
                    title={isAlreadyCheckedIn ? "Pickup Photos (Already Submitted)" : `Vehicle Photos (${selectedImages.length}/5)`}
                    images={isAlreadyCheckedIn && existingCheckInData ? existingCheckInData.images : selectedImages}
                ...
                <View style={styles.actionButtons}>
                    {isAlreadyCheckedIn ? (
                        <Pressable onPress={handleProceedToReturn} style={styles.confirmButton}>
